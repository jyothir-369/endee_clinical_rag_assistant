...
# UPDATED FOR ENDEE
from services.endee_service import similarity_search_with_score
...
def retrieve(query: str, k: int = TOP_K) -> list[RetrievedChunk]:
    """
    Convert *query* to an embedding, search Endee,
...
# BONUS - UPDATED FOR ENDEE
def retrieve_relevant_chunks(query: str, k: int = 3) -> list[RetrievedChunk]:
